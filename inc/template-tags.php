...
if ( ! function_exists( 'twentyseventeen_posted_on_no_link' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function twentyseventeen_posted_on_no_link() {
	// Finally, let's write all of this to the page.
	echo '<span class="posted-on">' . twentyseventeen_time() . '</span><span class="byline"> ' . $byline . '</span>';
}
endif;
...
if ( ! function_exists( 'twentyseventeen_time' ) ) :
/**
 * Gets a nicely formatted string for the published date.
 */
function twentyseventeen_time() {
	$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
	}
...
function twentyseventeen_entry_footer() {
	...
	if ( ( ( twentyseventeen_categorized_blog() && $categories_list ) || $tags_list ) || get_edit_post_link() ) {
		...
		echo '<div class="entry-supplemental-section entry-supplemental-section__no-border">';
		...
		echo '</div> <!-- .entry-supplemental-section -->';
	}
}
endif;
